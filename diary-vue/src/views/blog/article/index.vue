<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          v-hasPermi="['blog:article:create']"
          plain
          type="primary"
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column align="center" label="封面文件" prop="coverFileId" width="110">
        <template #default="{ row }">
          <el-tag v-if="row.coverFileId" size="small">#{{ row.coverFileId }}</el-tag>
          <span v-else>-</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" fixed="right" label="操作" width="180">
        <template #default="{ row }">
          <el-button
            v-hasPermi="['blog:article:update']"
            link
            type="primary"
          ...
          <el-button
            v-hasPermi="['blog:article:delete']"
            link
            type="danger"
...
const queryParams = reactive<BlogApi.BlogArticlePageReqVO>({
  pageNo: 1,
  pageSize: 10,
  title: undefined,
  status: undefined,
...
const getList = async () => {
  loading.value = true
  try {
    const data = await BlogApi.getBlogArticlePage(queryParams)
...
const openEditor = (type: 'create' | 'update', id?: number) => {
  if (type === 'create') {
    router.push('/content/blog/create')
    return
  }
  router.push({ path: '/content/blog/edit', query: { id } })
}

const handleDelete = async (id: number) => {
  try {
    await message.delConfirm()
    await BlogApi.deleteBlogArticle(id)

...
| 2026-06-30 | 第二阶段 | 接入 RocketMQ 事件、Redis 消费幂等、file outbox 与补偿重试 | `mvn "-Dmaven.repo.local=..\.m2\repository" -pl diary-framework/diary-spring-boot-starter-mq -am test "-Dtest=RedisMessageIdempotentServiceTest,RocketMqDomainEventPublisherTest" -DfailIfNoTests=false "-Dsurefire.failIfNoSpecifiedTests=false"` 通过；`mvn "-Dmaven.repo.local=..\.m2\repository" -pl diary-module-file -am test "-Dtest=FileOutboxEventServiceTest,FileObjectServiceImplTest" -DfailIfNoTests=false "-Dsurefire.failIfNoSpecifiedTests=false"` 通过 | 新增 `diary-spring-boot-starter-mq`；文件上传后写 `file_outbox_event`，调度器发送 `diary-file-events:file-uploaded`，消费者用 Redis `mq:idempotent:*` 做幂等；Compose 已包含 RocketMQ，实际容器启动仍需在安装 Docker 的环境复验 |
| 2026-06-30 | 第三阶段 | 落地 Sentinel 文件规则、Gateway 降级响应和 Feign fallback | `mvn "-Dmaven.repo.local=..\.m2\repository" -pl diary-module-file-api -am test "-Dtest=FileObjectApiFallbackFactoryTest" -DfailIfNoTests=false "-Dsurefire.failIfNoSpecifiedTests=false"` 通过；`mvn "-Dmaven.repo.local=..\.m2\repository" -pl diary-gateway,diary-module-file-api,diary-module-diary,diary-module-blog -am test -DskipTests` 通过 | diary/blog 启用 `feign.sentinel.enabled` 并加载 classpath Sentinel flow/degrade 规则；Gateway 配置 Sentinel 429 JSON fallback；`FileObjectApiFallbackFactory` 返回可识别的 `file-service unavailable` 降级结果；`infra/sentinel/README.md` 记录演练方式 |
| 2026-06-30 | 第一/三阶段 | 增加 Compose 启动运行手册和登录/上传/列表冒烟验收脚本 | PowerShell 脚本语法检查通过：`[scriptblock]::Create((Get-Content -Raw infra\smoke\smoke.ps1))`；本机执行 `docker --version` 失败，无法实际启动 Compose | 新增 `docs/microservices/runbook.md` 和 `infra/smoke/smoke.ps1`；脚本覆盖 Gateway 健康、登录、文件上传、日记列表、博客列表；真实 Compose 与端到端验收需在安装 Docker 的环境执行 |
